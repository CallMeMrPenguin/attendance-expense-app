...
async function listTables() {
  ...
  const { data: budgets } = await client.from('category_budgets').select('*');
  console.log('category_budgets:', budgets);

  const { data: funds } = await client.from('savings_funds').select('*');
  console.log('savings_funds:', funds);
}
