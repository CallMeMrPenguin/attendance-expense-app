...
async function main() {
  console.log('Testing user_table_settings table in Supabase...');
  const { data, error } = await supabase.from('user_table_settings').select('*').limit(1);
  if (error) {
    console.log('user_table_settings error:', error.message);
  } else {
    console.log('user_table_settings exists! Rows sample:', data);
  }
}
